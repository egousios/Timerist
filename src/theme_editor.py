...
from PyQt5.QtWidgets import QComboBox, QDialog, QFormLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QListWidget, QMainWindow, QMessageBox, QPushButton, QScrollArea, QSizePolicy, QSpacerItem, QToolButton, QVBoxLayout, QWidget
...
class SaveColorThemeData:
    """
    Json-File objects holding information about saved
    workspaces in the ThemeEditor.
    """
...
class ThemeEditor(QMainWindow):
    ...
    def makeMenu(self):
        ...
        self.themes_searchbar.setFixedWidth(menu_title.width()/2)
        ...
        self.themes_list.addItems(get_updated_themes_list())

        self.edit_action = QPushButton("Edit")
        self.edit_action.setFont(self.btn_font)
        self.edit_action.clicked.connect(self.edit_theme)
        ...
        self.actions_widget_layout.addWidget(self.edit_action)
    ...
    def new_theme(self):
        ...
        self.color_theme_name.setText("")
        self.submit_theme_btn.setDisabled(False)
        self.publish_theme_btn.setDisabled(False)
        self.submit_theme_btn.clicked.connect(self.submit_new_theme)
        # self.publish_theme_btn.clicked.connect(self.publish_new_theme)
        self.go_back()

    def submit_new_theme(self):
    ...
    def edit_theme(self):
        if self.selected_theme_from_menu != None:
            ...
            theme_lighter_color = saved_theme_data["theme_lighter_color"]
            theme_darker_color = saved_theme_data["theme_darker_color"]
            theme_data_filepath = saved_theme_data["theme_data_filepath"]
            ...
            self.submit_theme_btn.setDisabled(False)
            self.publish_theme_btn.setDisabled(False)
            self.publish_theme_btn.clicked.connect(self.publish_edited_theme)
            self.go_back()
    ...
    def submit_theme(self):
        if self.selected_theme_type == "Color/Element Theme":
            with open(self.color_base_theme, "r") as f:
                ...
                f.close()
            self.theme_file_name = self.color_theme_name.text()
            self.lighter_color = self.selected_lighter_color
            self.darker_color = self.selected_darker_color
            self.theme_file_data = data.replace("#bf84f0", self.lighter_color)
            self.theme_file_data = self.theme_file_data.replace("#994e93", self.darker_color)
            print(self.lighter_color, self.darker_color)
        elif self.selected_theme_type == "Custom Theme":
            pass

    def publish_edited_theme(self):
        pass
