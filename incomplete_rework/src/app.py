...
from PyQt5.QtGui import QCloseEvent, QCursor, QDesktopServices, QFont, QIcon, QKeySequence, QMovie, QFontDatabase, QPainter, QPixmap, QStandardItem, QStandardItemModel, QTextImageFormat, QTextListFormat, QTextTableFormat, QTextCursor
    ...
    def save(self):
        ...
        if os.path.isfile(selected):
            ...
            with open(selected, "a", encoding='utf-8') as f:
                ...
                f.close()
            if self.database != None:
                self.database.addRow(QStandardItem(f'{self.lineEdit.text()}.html'))
...
class Ui_Timerist(object):
    def setupUi(self, Timerist, sound, email, password):
        ...
        self.treeWidgetFilter = QtWidgets.QLineEdit()
        self.treeWidgetFilter.setPlaceholderText("Search...")

        self.treeWidgetFilterBtn = QtWidgets.QToolButton()
        self.treeWidgetFilterBtn.setToolTip("Search")
        self.treeWidgetFilterBtn.setIcon(QIcon("../images/search.png"))
        self.treeWidgetFilterBtn.setIconSize(self.tool_btn_size_2)
        self.treeWidgetFilterBtn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.treeWidgetFilterBtn.clicked.connect(self.filterTree)

        self.documentDatabaseFilter = QtWidgets.QLineEdit()
        self.documentDatabaseFilter.setPlaceholderText("Search...")

        self.documentDatabaseFilterBtn = QtWidgets.QToolButton()
        self.documentDatabaseFilterBtn.setToolTip("Search")
        self.documentDatabaseFilterBtn.setIcon(QIcon("../images/search.png"))
        self.documentDatabaseFilterBtn.setIconSize(self.tool_btn_size_2)
        self.documentDatabaseFilterBtn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.documentDatabaseFilterBtn.clicked.connect(self.filterDocuments)
        ...
        self.documentsDatabase = QtWidgets.QListView(self.MainWidget)
        self.documentsDatabase.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.documents_model = QStandardItemModel()
        self.documents_proxy_model = QSortFilterProxyModel()
        self.documentsDatabase.setModel(self.documents_proxy_model)
        self.documents_proxy_model.setSourceModel(self.documents_model)
        ...
        BottomLayout.addWidget(self.treeWidgetFilter, 90)
        BottomLayout.addWidget(self.treeWidgetFilterBtn, 10)
        searchWidget.setLayout(BottomLayout)
        ####################################
        BottomLayout2 = QHBoxLayout() 
        searchWidget2 = QtWidgets.QWidget()
        BottomLayout2.addWidget(self.documentDatabaseFilter, 90)
        BottomLayout2.addWidget(self.documentDatabaseFilterBtn, 10)
        searchWidget2.setLayout(BottomLayout2)
        self.TodoOptionsLayout.addLayout(BottomLayout)
        ...
        verticalsearch = QVBoxLayout()
        verticalsearch.addLayout(self.TodoOptionsLayout)
        verticalsearch.addWidget(searchWidget)
        self.TodoLayout.addLayout(verticalsearch)
        ...
        verticalsearch2 = QVBoxLayout()
        verticalsearch2.addLayout(self.documentsOptionsLayout)
        verticalsearch2.addWidget(searchWidget2)
        self.documentsLayout.addLayout(verticalsearch2)
    ...
    def filterDocuments(self):
        text = self.documentDatabaseFilter.text()
        search = QtCore.QRegExp(text, Qt.CaseSensitivity.CaseSensitive, QRegExp.RegExp)
        self.documents_proxy_model.setFilterRegExp(search)
    ...
    def remove(self):
        ...
        if ask == QtWidgets.QMessageBox.Yes:
            for item in self.documentsDatabase.selectionModel().selectedIndexes():
                items.append(self.documents_model.itemFromIndex(item).text())
                if len(items) >= 1:
                    self.documents_model.takeItem(item.row())
    ...
    def Createdocument(self):
        create = CreateWindow(Timerist, database=self.documentsDatabase.model(), user=self.email)
        create.show()
